...
%========================================================================== 
% JDQR 
%   Computes a partial Schur decomposition of a square matrix or 
%   operator. 
%==========================================================================
%   Lambda = JDQR(A) returns the absolute largest eigenvalues in a K vector
%   Lambda. Here K = min(5,N) (unless K has been specified), where 
%   N = size(A,1). JDQR(A) (without output argument) displays the K 
%   eigenvalues.
%
%   [X, Lambda] = JDQR(A) returns the eigenvectors in the N by K matrix X 
%   and the eigenvalues in the K by K diagonal matrix Lambda. Lambda 
%   contains the Jordan structure if there are multiple eigenvalues.
%
...
%   S is an K by K upper triangular matrix and Q is an N by K orthonormal 
%   matrix such that A*Q = Q*S. The diagonal elements of S are eigenvalues 
%   of A.
%
...
%   For example, EIGS('fft',...) is much faster than EIGS(F,...) where F is
...
%
%   If SIGMA is not specified, then the K-th eigenvalues largest in magni-
%   tude are computed. If SIGMA is zero, then the K-th eigenvalues smallest 
%   in magnitude are computed.  If SIGMA is a real or complex scalar then 
%   the K-th eigenvalues nearest SIGMA are computed.  If SIGMA is one of 
%   the following strings, then it specifies the desired eigenvalues.
%
...
%   'BE' 	Both Ends. Computes k/2 eigenvalues from each end of the spec-
%           trum (one more from the high end if k is odd.)
%
...
% Author   	: (C) Gerard Sleijpen, 1998
% Revised   : (C) Laurens van der Maaten, 2010
%             UC San Diego / Delft University of Technology
% Coypright : This file is part of the Matlab Toolbox for Dimensionality 
%             Reduction v0.7.2b. The toolbox can be obtained from 
%                   http://homepage.tudelft.nl/19j49
%             You are free to use, change, or redistribute this code in any
%             way you want for non-commercial purposes. However, it is 
%             appreciated if you maintain the name of the original author.
% Created	: 1998
% Last Revised	: Dec 23, 2014 at 10:01 by Karamatou Yacoubou Djima
%==========================================================================
