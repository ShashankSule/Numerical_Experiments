...
% University California, San Diego / Delft University of Technology
    ...
    switch mapping.name
        ...
        case 'KernelPCAfast'
            % Compute and center kernel matrix
            gapsize=min(2000,size(point,1));
            t_point=zeros(size(point,1),size(mapping.V,2));
            h=waitbar(0,'Working');
            invsqrtLXVTT=(mapping.invsqrtL * mapping.V')';
            
            sumA=sum(mapping.X'.*mapping.X')';
            
            for i=1:gapsize:size(point,1)
                K = gram(mapping.X, point(i:min(i+gapsize,size(point,1)),:), mapping.kernel, mapping.param1, mapping.param2,sumA);
                
                %J = repmat(mapping.column_sums', [1 size(K, 2)]);
                %K = K - repmat(sum(K, 1), [size(K, 1) 1]) - J + repmat(mapping.total_sums, [size(K, 1) size(K, 2)]);
                
                
                K=bsxfun(@minus,bsxfun(@minus,K,sum(K,1)),mapping.column_sums')+mapping.total_sums;
                
                % Compute transformed points
                
                %t_point(i:min(i+gapsize,size(point,1)),:) = [mapping.invsqrtL * mapping.V' * K]';
                t_point(i:min(i+gapsize,size(point,1)),:) =  K'*invsqrtLXVTT;
                
                
                %t_point = t_point';
                fprintf('%d\n',i)
                waitbar(i/size(point,1));
            end
            close(h)
        ...
        case 'Laplacian'         
            ...
            % Compute embeddings
            t_point = repmat(0, [size(point, 1) numel(mapping.val)]);
            KK = (L2_distance(point', mapping.X') .^ 2)';
            [foo IND]=sort(KK, 'ascend');            
            for i=1:size(point, 1)
                
                % Compute Gaussian kernel between test point and training points
                K=KK(:,i);
                K(IND(mapping.k+1:end,i)) = 0;
                K=K/max(K);
