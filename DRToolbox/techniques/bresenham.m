...
if numel(mycoords==2) %added by tim to allow for 1 dimensional coordinates as input
[x0,x1] = ind2sub(size(mymat),round(mycoords(1)));
x=[x0;x1];
[y0,y1]= ind2sub(size(mymat),round(mycoords(2)));
y=[y0;y1];
else
x = round(mycoords(:,1));
y = round(mycoords(:,2));
