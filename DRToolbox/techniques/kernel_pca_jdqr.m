function [mappedX, mapping] = kernel_pca_jdqr(X, no_dims, varargin)
...
% University California, San Diego / Delft University of Technology
    ...
    global dataX column_sums total_sum verbose widthparam
    verbose = false;
    ...
    if nargin > 2
		kernel = varargin{1};
		if length(varargin) > 1 & strcmp(class(varargin{2}), 'double'), widthparam = varargin{2}; end
    ...
    if size(X, 1) < 2000
        ...
        column_sums=[];
        total_sum=[];
        [V,L]=jdqr('jdqrKernelFunction', size(X,1), no_dims, 'LM', options);
        mapping.column_sums=column_sums/ell;
        mapping.total_sums=sum(column_sums)/ell;
        disp(' ');
    end
